...
class DeepBCFRAgent():
    def __init__(self,
        ...
        self.regretNN = [[CFREstimator(inputsize,self.env.num_actions,epochs=epochs,mini_batches=mini_batches,isRegret=True) for _ in range(self.m)],
                         [CFREstimator(inputsize,self.env.num_actions,epochs=epochs,mini_batches=mini_batches,isRegret=True) for _ in range(self.m)]]
        self.regretMemory = [[Memory(self.env.card2index, memory_size=memory_size, batch_size=batch_size) for _ in range(self.m)],
                             [Memory(self.env.card2index, memory_size=memory_size, batch_size=batch_size) for _ in range(self.m)]]
        ...
        for t in range(self.T):
            for typ in range(self.m):
                ...
                for player_id in range(self.env.num_players):
                    ...
                    # print(player_id,typ,len(regretMemory))
    ...
    def update_regret(self, player_id):
        ...
        '''
        # if player_id == 0:
        #     # self.regretNN.train(self.regretMemory[player_id], self.iteration)
        #     self.regretNN[player_id].train(self.regretMemory[player_id], self.iteration)
        # else:
        #     # for i in range(self.m):
        self.regretNN[player_id][self.tmp_typ].train(self.regretMemory[player_id][self.tmp_typ], self.iteration)
    ...
    def traverse_tree(self, probs, player_id, regretMemory, policyMemory):
        ...
        # print(current_player,player_id)
    ...
    def feed(self, state, player_id, isRegret=True):
        ...
        if isRegret==True:
            ...
            # else:
            # print(state)
            self.regretMemory[player_id][self.tmp_typ].save(state)
        else:
    ...
    def action_probs(self, state, legal_actions, isEval=False):
        ...
        if not isEval:
            ...
            # if self.env.get_player_id() == 0:
            #     regret = self.regretNN[self.env.get_player_id()].predict(x_test)[0].numpy()
            # else:
            for i in range(self.m):
                # print(self.regretNN[self.env.get_player_id()][i].predict(x_test).numpy())
                tmp_regret = self.p[i]*self.regretNN[self.env.get_player_id()][i].predict(x_test)[0].numpy()
                regret += tmp_regret
